...
export default async function handler(request, response) {
  ...
  try {
    ...
    const prompt = "Generate a complete English placement test for a CEFR evaluation, ranging from A1 to B2 levels. The test must be in a valid JSON format according to the provided schema. It must include exactly: 10 grammar questions, 10 proofreading questions (where the user identifies the error), 5 fill-in-the-blanks questions (where the user types the missing word), 2 different reading comprehension passages with 3 multiple-choice questions each, 1 listening comprehension text (a short story of about 150 words) with 5 multiple-choice questions, and one essay prompt. For fill-in-the-blanks, use '[BLANK]' as the placeholder in the question string. All content must be in English.";
    const schema = {
        type: "OBJECT",
        properties: {
            ...
            "fill-in-the-blanks": { type: "ARRAY", items: { type: "OBJECT", properties: { question: { type: "STRING" }, answer: { type: "STRING" } } } },
